...
GameObject::GameObject(uint16_t height, uint16_t width, uint16_t pos_x, uint16_t pos_y, uint8_t speed) {

	if(width < 10) 			this->width = 10;
	else if (width > 700) 	this->width = 700;
	else 					this->width = width;

	if (height < 10) 		this->height = 10;
	else if (height > 100) 	this->height = 100;
	else					this->height = height;

	//TODO Not sure what speed limitation should be
	if (speed < 1) 			this->speed = 1;
	else if (speed > 30) 	this->speed = 30;
...
void GameObject::setWidth(uint16_t width) {
	if(width < 10) 			this->width = 10;
	else if (width > 700) 	this->height = 700;
	else 					this->width = width;
}
